    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid xl:grid-cols-2 gap-6">
                <div>
                    <h2 class="text-2xl mb-4">Удобства комнат</h2>
                                        ...
                                        @foreach($room->facilities as $facility)
                                            <li>
                                                <div class="inline-flex gap-4">
                                                    ...
                                                    <form class="w-auto" action="{{ route('admin.rooms.facility.del', ['room' => $room->id, 'facility' => $facility->id]) }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                ...
                <div>
                    ...
                    <form action="{{ route('admin.rooms.facility') }}" method="post">
                        @csrf
                        <div class="mb-5">
                            <x-label for="room">Выбрать комнату</x-label>
                            <select id="room" name="room" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option @if(!old('room')) selected @endif disabled>Комната</option>
                                @foreach($rooms as $room)
                                    <option @if($room->id === old('room')) selected @endif  value="{{ $room->id }}">{{ $room->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-5">
